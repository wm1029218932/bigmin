...
!function (e, t) {
    ...
    function u(e, t, n) {
        ...
        if (void 0 === n && 1 === e.nodeType)
            if (r = "data-" + t.replace(Dt, "-$&").toLowerCase(), n = e.getAttribute(r), "string" == typeof n) {
                try {
    ...
    var Ut, Vt = {HTML: "block", BODY: "block"},
        Yt = /^margin/,
...
$(function () {
    ...
    var biaoChannel = getUrlParam('biao') || '双12-抽红包';
    var mtChannel = getUrlParam('mt') || '17';
    var cdbChannel = getUrlParam('cdb') || '1';


    if (!!getCookie('zpcount')) {
        if (getCookie('zptime') != new Date().getDate()) {
            setCookie('zpcount', '3', 'd1');
            setCookie('zptime', new Date().getDate(), 'd1')
        }
    } else {
        setCookie('zpcount', '3', 'd1');
        setCookie('zptime', new Date().getDate(), 'd1')
    ...
    $(".needCredits span").html('今日剩余' + getCookie('zpcount') + '次抽奖机会');
...
//		againOrderId && (data.againOrderId = againOrderId), window.xhr = $.ajax({ url: CFG.doJoin, type: "post", dataType: "json", data: data, 
//		success: function(result) {
//			window.lastResult = result, 
//			eval(result.token), 
//			result.success ? (againOrderId = null, postOrder(result.orderId)) : 1 == result.status ? rotateError("noLogin", result) : 2 == result.status ? rotateError("noCredits", result) : 3 == result.status ? rotateError("todayOver") : 4 == result.status ? rotateError("over") : 5 == result.status ? rotateError("gameClose") : rotateError("systemError", result) }, 
//			error: function() {alert(1);!timeout && rotateError("networkError") } }) 
    ...
    function postOrder(t) {
        setTimeout(function () {
//		window.xhr = $.ajax({ url: CFG.quireOrder, type: "post", dataType: "json", data: { orderid: t }, 
//		success: function(e) { timeout || (window.lastResult = e, e.success ? -1 == e.result ? postOrder(t) : (e.againTag && (againOrderId = e.againTag), $(".needCredits span").html(e.needCreditsText), rotateFn(e)) : rotateError("systemError", e)) }, error: function() {!timeout && rotateError("networkError") } }) }, 1e3) 
            var now = function () {
                var t = new Date();
                var y = t.getFullYear();
            ...
            var yxq = '<p style="color:#3a3a3a;padding-top:.1rem;">有效期：' + now() + '</p>'


            var p = [0, 0, 0, 1, 1, 1, 2, 2, 3, 3];
            if (getUrlParam('mt') == '14') {
                p = [1, 1, 1, 1, 1, 1, 1, 2, 2, 3]
            }
            var r = p[Math.floor(Math.random() * 10)];
//		r= 2;
            var s = [{
                id: '1',
                result: '2',
                title: '幸运礼包',
//			imgurl: 'zpSrc/image/话费.png',
//			link: 'http://www.baidu.com',
                type: 'coupon',	//优惠券
                oo: [{
                    title: '手表0.4折购买特权！',
                    desc: '女人看包，男人看表！真男人怎么可以没有一块高档手表！0.4折还有5大豪礼免费相赠，进店领取福利把！点击马上使用！' + yxq,
                    link: 'http://www.doumobi.net/jixiebiao2.html?channel=' + biaoChannel,
                    imgurl: 'zpSrc/image/l-shoubiao.jpg'
                }, {
                    title: '智能充电宝0.12折购买特权！',
                    desc: '智能轻薄背夹充电宝，大容量，无下巴！4颗LED精准显示电量。0.12折还有豪华礼包相赠，，进店领取福利把！点击马上使用！' + yxq,
                    link: 'http://www.doumobi.net/chongdian.html?channel=' + cdbChannel,
                    imgurl: 'zpSrc/image/l-chongdianbao.jpg'
                }]
            }, {
                id: '2',
                result: '2',
                title: '免费领取茅台酒',
                imgurl: 'zpSrc/image/l-maotai.jpg',
                link: 'http://www.doumobi.net/maotaiyuanjiang.html?channel=' + mtChannel,
                desc: '三分酿造，七分原料。贵州茅台原浆酒有着丰富的历史文化，深厚的文化积淀，是馈赠亲友的上等之选。统一零售价698元/瓶，点击马上免费领取！' + yxq
            }, {
                id: '5',
                result: '1',
            }, {
            ...
            var e = {
                needCreditsText: '抽奖中',
                result: s[r].result,	//结果 0：未中奖 1：再来一次 2:中奖
                lottery: {
                    id: s[r].id,	//中奖ID(data-id)
                    title: s[r].title,
                    imgurl: s[r].imgurl,
                    link: s[r].link,
                    desc: s[r].desc,
...
//		$(".needCredits span").html(e.needCreditsText), 
            rotateFn(e);
        }, 1e3)
    }
    ...
    function showModal(t, e) {
        var o = "",
            ...
            a = "",
            l = {
                over: "zpSrc/image/over.png",
                networkError: "zpSrc/image/networkError.png",
                noCredits: "zpSrc/image/noCredits.png",
                noPrize: ["zpSrc/image/noPrize1.png", "zpSrc/image/noPrize2.png"],
                again: "zpSrc/image/again.png"
            };
        switch (t) {
            case "systemError":
            ...
            case "over":
                ...
                break;
            case "todayOver":
                o = "今日已抽完", r = "转发微信可继续抽奖或明天再来~", n = l.over, i = !1, a = ".know";
                break;
            case "noPrize":
                var c = l.noPrize;
                ...
                setCookie('zpcount', getCookie('zpcount') - 1, 'd1');
                break;
            case "again":
            ...
            case "coupon":
                var rr = Math.floor(Math.random() * e.lottery.oo.length);
                var oo = e.lottery.oo;

                o = "恭喜您", r = oo[rr].title, n = oo[rr].imgurl,
                    i = !0,
                    a = ".prize-continue,.getPrize",
                    $('.block-help').html(oo[rr].desc);
                setCookie('zpcount', getCookie('zpcount') - 1, 'd1');
                $(".getPrize").off("click").on("click", function () {
                    window.location.href = oo[rr].link
                })
                break;

            case "prize":
                setCookie('zpcount', getCookie('zpcount') - 1, 'd1');
                $('.block-help').html(e.lottery.desc);
                o = "恭喜您", r = e.lottery.title, n = e.lottery.imgurl, i = !0, a = ".prize-continue,.getPrize", $(".getPrize").off("click").on("click", function () {
                    window.location.href = e.lottery.link
                })
        }
    ...
    var timeout = !1,
        ...
        rotateStart = function () {
            timeout = !1,
                running = !0,
                $("#circle").rotate({
                    angle: 0, animateTo: 2160, duration: 8e3, easing: function (t, e, o, r, n) {
                        return -r * (e /= n) * (e - 2) + o
                    },
                    callback: function () {
//					timeout = !0, window.xhr && window.xhr.abort(), showModal("networkError") 
        ...
        rotateFn = function (t) {
            ...
            $("#circle .prize").each(function () {
                ...
                duration: 4e3,
                callback: function () {
                    0 == t.result ? showModal("noPrize") : 1 == t.result ? showModal("again") : "coupon" == t.lottery.type ? showModal("coupon", t) : showModal("prize", t)
                }
            })
        };
    $("#start").click(function () {
        _hmt.push(['_trackEvent', 'start', 'click', '剩余次数:' + getCookie('zpcount')]);

        if (getCookie('zpcount') <= 0) {
            showModal("todayOver");
        } else {
            start()
        }
