...
function selectC(p,c) {
	...
	if (sCity != "省直辖县级行政单位") {
		if (sCity == "东莞市" || sCity == "中山市" || sCity == "儋州市" || sCity == "嘉峪关市") {
			expressArea += sCity;
			if($('#areaList').hasClass('select-tbr')){
				$("#expressArea1 .choose-bg").val(expressArea);
			}else if($('#areaList').hasClass('select-bbr')){
				$("#expressArea2 .choose-bg").val(expressArea);
			}
			clockArea();
...
$(function() {
	...
	$("#expressArea1").click(function() {
		$('#areaList').removeClass('select-bbr').addClass('select-tbr');
		$("#areaMask").fadeIn();
		$("#areaLayer").animate({"bottom": 0});
	});
	$("#expressArea2").click(function() {
		$('#areaList').removeClass('select-tbr').addClass('select-bbr');
		$("#areaMask").fadeIn();
		$("#areaLayer").animate({"bottom": 0});
	});
	/*关闭省市区选项*/
